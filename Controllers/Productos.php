...
{
    // se agrego la funcion detalle del producto
    public function verDetalle($idProducto)
    {
        // Verificar si $idProducto es un número
        if (is_numeric($idProducto)) {
            // Obtener detalles del producto desde el modelo
            $detalleProducto = $this->model->getProducto($idProducto);

            // Verificar si se obtuvieron detalles del producto
            if ($detalleProducto) {
                $data['title'] = 'Detalle del Producto';
                $data['producto'] = $detalleProducto;

                // Cargar la vista de detalle del producto
                $this->views->getView('detalle_producto', 'index', $data);
            } else {
                // Manejar el caso en que el producto no se encuentra
                echo "Producto no encontrado.";
            }
        } else {
            // Manejar el caso en que $idProducto no es un número
